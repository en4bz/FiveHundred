...
public class CopyOfBasicBiddingStrategy implements IBiddingStrategy
{
	...
	// Counts the number of cards in a  particular suit given that the robot will bid a trump bid
	private static EnumMap<Suit, Integer> aSuitNumberCardsTrump = new EnumMap<Suit, Integer>(Suit.class);
	 ...
	 * 
	 */
	public CopyOfBasicBiddingStrategy()
	{
	...
	public Bid selectBid(Bid[] pPreviousBids, Hand pHand)
	{
		...
		// STEP 1: Check each suit one by one and count the points according to
	    // the declared point scheme

	    updatePointsOfHandTrumpGame(pHand);
	    
	    
	    updatePointsOfHandNoTrumpGame(pHand);
	...
	/**
	 * Updates the points of a trump game bid for each of the suits.
	 * @param pHand The hand of the robot
	 */
    private static void updatePointsOfHandTrumpGame(Hand pHand)
    {
    	
		// Updates the points for a trump bid
		for (int i = 0; i < SUITS.length; i++)
		{
			
			// The cardlist containing all possible trump cards
			CardList allTrumpCards = pHand.getTrumpCards(SUITS[i]);
			pHand.
			int pointsInSuit = countPointsInSuit(allTrumpCards, SUITS[i], true);
			// Base points
			aSuitPointsTrump.put(SUITS[i], pointsInSuit);
	...
	/**
	 * Updates the points of a no trump game.
	 * @param pHand The hand of the robot
	 */
	private static void updatePointsOfHandNoTrumpGame(Hand pHand)
	{
		
		// Resets the total no trump points
		totalNoTrumpPoints = 0;
		...
		for (int i = 0; i < SUITS.length; i++)
		{
			...
			totalNoTrumpPoints =  totalNoTrumpPoints + noTrumpPoints;
	 ...
	 * @param pNoTrump True if we are checking for a no trump game, false otherwise
	...
	private static int countPointsInSuit(CardList pCardsSuit, Suit pSuit, boolean pTrump)
	{
		...
		// If the cards are being sorted according to no trump
		if (pNoTrump)
		{
